...
# Set Stripe API keys
STRIPE_API_KEY = "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"
...
conf = ConnectionConfig(
    MAIL_USERNAME="Yosrizayani@gmail.com",
    MAIL_PASSWORD="jbomvyfqjcxtixrz",
    MAIL_FROM="Yosrizayani@gmail.com",
    MAIL_PORT=465,
...
# Model for Stripe payment creation
class CreateOrder(BaseModel):
    amount: int
    currency: str = "tnd"  # Default currency
...
async def create_order(input: CreateOrder, id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        payment_intent = stripe.PaymentIntent.create(
            amount=input.amount * 100,  # Stripe accepts amounts in the smallest currency unit
            currency=input.currency,
        )

        # Save payment details in the database
        patient.order_id = payment_intent.id
        patient.amount = payment_intent.amount
...
@router.get("/verify_order/{order_id}", tags=["patients"])
async def verify_order(order_id: str, db: Session = Depends(get_db)):
    try:
        ...
        if payment_intent['status'] == 'succeeded':
            ...
            return JSONResponse(status_code=500, content={'message': 'Payment not successful'})
    except Exception as e:
